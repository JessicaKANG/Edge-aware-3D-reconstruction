...
G2OEngine::G2OEngine(
  ...
  if (!sLogging_file_.empty())
  {
    ...
    html_doc_stream_->pushInfo(
      htmlDocument::htmlMarkup("h1", std::string("G2OEngine")));
...
void G2OEngine::SetFeaturesProvider(Features_Provider * provider)
{
...
void G2OEngine::SetMatchesProvider(Matches_Provider * provider)
{
  matches_provider_ = provider;
}

void G2OEngine::SetRotationAveragingMethod
(
...
void G2OEngine::SetTranslationAveragingMethod
(
...
bool G2OEngine::Compute_Global_Rotations
(
...
bool G2OEngine::Compute_Global_Translations
(
...
bool G2OEngine::Compute_Initial_Structure
(
  matching::PairWiseMatches & tripletWise_matches
)
{
...
bool G2OEngine::Adjust()
{
  // Export initial structure
  if (!sLogging_file_.empty())
  {
    Save(sfm_data_,
      stlplus::create_filespec(stlplus::folder_part(sLogging_file_), "structure_00_before_BA", "ply"),
      ESfM_Data(EXTRINSICS | STRUCTURE));
    Save(sfm_data_,
      stlplus::create_filespec(stlplus::folder_part(sLogging_file_), "sfm_data_before_BA", ".json"),
      ESfM_Data(ALL));
  }
  
  Bundle_Adjustment_G2O bundle_adjustment_obj;
  
  // Refine sfm_scene (2 times):
  // --
  // 1st BA
  // --
  bool b_BA_Status = bundle_adjustment_obj.Adjust(sfm_data_, true, true, 20);

 
  if (b_BA_Status && !sLogging_file_.empty())
  {
    Save(sfm_data_,
      stlplus::create_filespec(stlplus::folder_part(sLogging_file_), "structure_01_1st_BA", "ply"),
      ESfM_Data(EXTRINSICS | STRUCTURE));
  }
  /*
  ...
  {
    Save(sfm_data_,
      stlplus::create_filespec(stlplus::folder_part(sLogging_file_), "structure_02_outlier_removed", "ply"),
      ESfM_Data(EXTRINSICS | STRUCTURE));
  }
  ...
  // --
  // 2nd BA. We refine one more time,
  // since some outlier have been removed and so a better solution can be found.
  //--
  b_BA_Status = bundle_adjustment_obj.Adjust(sfm_data_, true, true, 20);
  
    
  if (b_BA_Status && !sLogging_file_.empty())
  {
    Save(sfm_data_,
      stlplus::create_filespec(stlplus::folder_part(sLogging_file_), "structure_03_2nd_BA", "ply"),
      ESfM_Data(EXTRINSICS | STRUCTURE));
  }*/
...
// Adjust the scene (& remove outliers) with params
bool G2OEngine::Adjust(std::map <std::pair<IndexT, std::pair<double, double>>, double> map_angle,std::pair<double, double> param_pair)
{
  // Export initial structure
  if (!sLogging_file_.empty())
  {
  ...
  // 1st BA
  // --
  bool b_BA_Status = bundle_adjustment_obj.Adjust(sfm_data_, true, true, 20, map_angle, param_pair);
...
void G2OEngine::Compute_Relative_Rotations
(
